...
func (s *AuthService) HandleOAuthCallback(ctx context.Context, provider domain.AuthProvider, code, state string) (*AuthResponse, error) {
...
func (s *AuthService) generateAuthResponse(user *domain.User) (*AuthResponse, error) {
	...
	return &AuthResponse{
		...
		ExpiresAt:    0, // Will be set by handler based on JWT expiration
	}, nil
}
